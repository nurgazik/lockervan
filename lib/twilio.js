...
export async function sendPinToCustomer(phone, lockerId, netcode, rentalStart, rentalExpiry) {
  const timeOpts = { hour: 'numeric', minute: '2-digit', timeZone: 'America/Vancouver' };
  const start = new Date(rentalStart).toLocaleTimeString('en-CA', timeOpts);
  const expiry = new Date(rentalExpiry).toLocaleTimeString('en-CA', timeOpts);
